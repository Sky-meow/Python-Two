            ...
            if f is None:
                ...
                if isinstance(f.remote_field, models.ManyToOneRel):
                    ...
                    if field_val is not None and field_val.name is not None:             
                        url = field_val.url
                        if 'http://' in str(field_val) or 'https://' in str(field_val):
                            url = str(field_val)
                            
                        item.text = mark_safe('<a href="%s" target="_blank" title="点击查看大图" data-gallery="gallery"><img src="%s" class="field_img"/></a>' % (url, url))
                    else:
                        item.text = '无'
                else:
